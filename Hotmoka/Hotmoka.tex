...
\begin{frame}\frametitle{The OpenZeppelin standard}
...
\begin{frame}\frametitle{The hierarchy of the implementation$^*$}
  ...
  \vspace*{-2ex}
  {\tiny $^*$ Joint implementation with Marco Crosara}
...
public class ERC20 {\color{red}extends Contract implements IERC20} \{
  ...
  public IERC20View snapshot() \{   {\color{violet}// O(1)}
    ...
    class SnapshotImpl {\color{red}extends Storage implements IERC20View} \{
...
\begin{frame}\frametitle{The view/snapshot design pattern}
  ...
  \begin{greenbox}{}
    ...
    the token and can then access its content, while other clients are
    modifying the original token
    \begin{itemize}
...
\begin{frame}\frametitle{A shared entity contract}
...
\}
  ...
  Generic types do not exist in Solidity
...
public interface {\color{red}SharedEntity<S extends PayableContract, O extends Offer<S>>
  ...
  {\color{red}@FromContract(PayableContract.class) @Payable} void place
    (BigInteger ticket, O offer);

  {\color{red}@FromContract(PayableContract.class) @Payable} void accept
    (BigInteger ticket, S buyer, O offer);
\}
  \end{semiverbatim}}
...
{\scriptsize\begin{semiverbatim}
public {\color{red}@FromContract @Payable} void place(BigInteger amount, O offer) \{
...
{\scriptsize\begin{semiverbatim}
public {\color{red}@FromContract @Payable} void accept(BigInteger amount, S buyer, O offer) \{
...
public final SharedEntityView<S> snapshot() \{
  ...
  class SharedEntitySnapshotImpl implements SharedEntityView<S> \{
    {\color{red}private final StorageMapView<S, BigInteger> snapshotOfShares = SimpleSharedEntity.this.snapshotOfShares;}
